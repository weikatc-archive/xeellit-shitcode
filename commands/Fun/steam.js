...
module.exports = {
    command: {
        description: 'поиск игры в стиме',
        usage: '<имя игры>',
        examples: {
            '{{prefix}}steam House Flipper': 'найдет игру с именем House Flipper в стиме'
        },
        permissions: {
            me: ['EMBED_LINKS']
        }
    ...
    execute: async function (message, args, options) {
        if (!args.length) return message.channel.send(`Ничего не забыл? Может нужно указать имя игры / программы? Не?`)
        const term = args.join(' ').slice(0, 256)

        const data = await fetch('https://store.steampowered.com/api/storesearch', { query: { term, l: 'ru', cc: 'ru' } }).then(games => games.items.shift())
        if (!data) return message.channel.send(`Я ничего не нашел по запросу **${term.slice(0, 100)}**.`)
        ...
        if (!gameData.success) return message.channel.send(`Не удалось удачно получить информацию о приложении **${data.name}**.`)
